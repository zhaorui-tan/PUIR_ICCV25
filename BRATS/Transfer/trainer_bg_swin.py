...
def write_fig(val_outputs_1, target, writer, epoch):
    # val_outputs_1 = [np.clip((i.detach().cpu().numpy() + 3) / 6, 0, 1) for i in val_outputs_1]
    # target_image = np.clip((target.detach().cpu().numpy() + 3) / 6, 0, 1)
    val_outputs_1 = [i.detach().cpu().numpy() for i in val_outputs_1]
    target_image = target.detach().cpu().numpy() 
    # print(target_image)

    # keys = ['t1c', 't1n', 't2w',  't2f', ] ###有点问题
